...
fn wrs_hist_animation() -> (usize, usize, usize) {
...
fn make_animations_in_python(
    ...
    let output = Command::new("python3")
        .arg("./visualizations_python/wrs_hist_animation.py")
    ...
    if !output.status.success() {
        println!(
            "Running wrs_hist_animation.py did not succeed. Error: \n\n{:#?}\n\n",
            output
        );
    } else {
        println!("{:#?}", output);
...
fn main() -> std::io::Result<()> {
    ...
    let (stream_size, capacity, step) = wrs_hist_animation();
